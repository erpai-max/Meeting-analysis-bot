...
# Configuration
# =======================
GCP_SERVICE_ACCOUNT_KEY = os.environ.get("GCP_SA_KEY")      # full JSON as string
...
# MIME/extension helpers for media detection
...
def authenticate_google_services() -> Tuple[Optional[object], Optional[gspread.Client]]:
    ...
    try:
        ...
        gsheets_client = gspread.authorize(creds)  # correct way to init gspread with creds
...
# Drive helpers
# =======================
def is_media_file(meta: Dict[str, str]) -> bool:
    mt = (meta.get("mimeType") or "").lower()
    name = (meta.get("name") or "").lower()
    by_mime = mt.startswith(AUDIO_PREFIXES) or mt.startswith(VIDEO_PREFIXES)
    by_ext = name.endswith(AUDIO_EXTS + VIDEO_EXTS)
    return by_mime or by_ext

def iter_folder_files(drive_service, folder_id: str) -> Iterable[Dict[str, str]]:
    """Yield file dicts (id, name, mimeType, parents) in a folder with pagination; filter media."""
...
def download_file(drive_service, file_id: str) -> io.BytesIO:
    ...
    while not done:
        ...
        if status is not None:
            # status can be None on final chunk
...
def transcribe_audio(file_content: io.BytesIO, original_filename: str) -> Optional[str]:
    ...
    # For Hinglish/Indian accents, "small" improves accuracy vs "tiny.en". Switch back if you need speed.
...
# =======================
KEY_ALIASES = {
    "Owner (Who handled the meeting)": "Owner (Who handled the meeting)",
    "Owner": "Owner (Who handled the meeting)",   # map short -> long if the sheet uses long
    "Action Items": "Action items",
    "Meeting Duration (min)": "Meeting duration (min)",
}
...
DEFAULT_HEADERS = [
    ...
    "Risks / Unresolved Issues", "Improvements Needed", "Owner (Who handled the meeting)",
    "Email Id", "Kibana ID", "Manager", "Product Pitch", "Team", "Media Link", "Doc Link",
...
def _backfill_scores(d):
    ...
    if not d.get("Total Score") or not d.get("% Score"):
        total = sum(_coerce_int(d.get(k, 0)) for k in keys)
        d["Total Score"] = str(total)
        d["% Score"] = f"{round((total/50)*100)}%"
    return d
...
def write_to_google_sheets(gsheets_client, data: Dict[str, str]) -> None:
    ...
    try:
        ...
        worksheet = spreadsheet.get_worksheet(0) or spreadsheet.sheet1

        headers = _ensure_headers(worksheet)

        # If sheet uses long Owner header, map; else keep short "Owner"
        sheet_has_long_owner = any(h.strip().lower().startswith("owner (who handled") for h in headers)
        normalized = {}
        for k, v in (data or {}).items():
            if sheet_has_long_owner:
                k2 = KEY_ALIASES.get(k, k)
            else:
                k2 = "Owner" if k.startswith("Owner") else k
            normalized[k2] = v

        normalized = _backfill_scores(normalized)
        row_to_insert = [normalized.get(h, "N/A") for h in headers]
        worksheet.append_row(row_to_insert, value_input_option="USER_ENTERED")
        logging.info(f"SUCCESS: Data for '{normalized.get('Society Name', 'N/A')}' written to Google Sheets.")
    except Exception as e:
